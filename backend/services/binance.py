...
def _get(path, params=None, ttl=CACHE_TTL):
    ...
    for base in (BINANCE_BASE_URL, BINANCE_FALLBACK_URL):
        if not base:
            continue
        try:
            r = requests.get(base.rstrip('/') + path, params=params, timeout=REQUEST_TIMEOUT, headers={'User-Agent':'OnchainAI/1.0'})
            r.raise_for_status()
            data = r.json()
            _cache[key] = (now, data)
            return data
...
def market_snapshot():
    ...
    return {'pairs': len(t), 'top_volume': t[:12], 'top_movers': movers[:12], 'updated_at': time.time(), 'provider': 'binance-public-market-data'}
